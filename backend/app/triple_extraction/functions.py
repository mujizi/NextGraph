...
async def _extract_single_chunk(chunk_text: str, retries: int = 3) -> List[Dict[str, str]]:
    ...
    system_prompt = (
        ...
        "6. **实体长度**：实体必须是短语而不是整句，优先提取核心名词或名词短语，通常不超过 15 个汉字或 8 个英文单词。\n"
        "7. **禁止事项**：不要把原因句、条件句、操作说明、完整现象描述、带有'因为/如果/当/请勿/否则/导致/用于/可以/需要'等结构的整段文本直接当成实体。\n"
        "8. **规范化**：如果文本里出现'温度控制未设置在合适位置'这类长描述，请只保留最核心的实体部分，例如'温度控制'或'合适位置'，不要保留整句。"
    )
